...
-- ============================================================
-- zotero-lite filter: uses Zotero native API, no BBT required
-- ============================================================
local json = require('lunajson')
local utils = require('utils')

local state = { fetched = nil, reported = {} }
local citekeys = {}
local config = { format = nil }
...
local function load_items()
  if state.fetched then return end
  state.fetched = { items = {}, meta = {} }

  local keys = {}
  for k, _ in pairs(citekeys) do table.insert(keys, k) end
  if #keys == 0 then return end

  local keysParam = table.concat(keys, ',')

  -- 单次请求获取完整元数据 (format=json 返回包含 itemID、key、citationKey 等)
  local url = 'http://127.0.0.1:23119/api/users/0/items?format=json&itemKey=' .. keysParam
  local mt, contents = pandoc.mediabag.fetch(url, '.')
  local ok, items = pcall(json.decode, contents)

  if not ok or type(items) ~= 'table' then
    print('zotero-lite: failed to fetch items: ' .. tostring(contents))
    state.fetched = { items = {}, meta = {} }
    return
  end

  -- 构建 itemKey -> {itemData, meta} 映射
  local keyToData = {}
  local keyToMeta = {}

  for _, item in ipairs(items) do
    local data = item.data or {}
    local itemKey = data.key or ''
    local citationKey = data.citationKey or ''

    if itemKey ~= '' then
      -- 存储完整 CSL 数据
      if item.id then
        keyToData[itemKey] = item
      end
      ...
      if itemKey ~= '' then
        local meta = {
          itemID = item.version,
          uri = 'http://zotero.org/users/' .. tostring(item.libraryID or 0) .. '/items/' .. itemKey,
        }
        keyToMeta[itemKey] = meta
      end
    end
  end

  -- 处理 citekey 可能是 itemKey 或 citationKey 的情况
  for citekey, _ in pairs(citekeys) do
    if keyToData[citekey] then
      -- 直接是 itemKey
      state.fetched.items[citekey] = keyToData[citekey]
      state.fetched.meta[citekey] = keyToMeta[citekey]
    else
      -- 尝试按 citationKey 反查
      for itemKey, item in pairs(keyToData) do
        local data = item.data or {}
        if data.citationKey == citekey then
          state.fetched.items[citekey] = item
          state.fetched.meta[citekey] = keyToMeta[itemKey]
          break
        end
      end
    end
...
local function get_item(id)
  load_items()
  if state.reported[id] then return nil end

  -- 直接查找
  if state.fetched.items[id] and state.fetched.meta[id] then
    return state.fetched.items[id], state.fetched.meta[id]
  end

  -- 尝试模糊匹配（兼容 citekey 为 citationKey 但存储为 itemKey 的情况）
  for itemKey, item in pairs(state.fetched.items) do
    local data = item.data or {}
    if data.citationKey == id then
      return item, state.fetched.meta[itemKey]
    end
  end

  state.reported[id] = true
  print('zotero-lite: @' .. id .. ' not found')
  return nil
end
...
local function zotero_ref(cite)
  ...
  for _, item in pairs(cite.citations) do
    ...
    if itemData == nil then return cite end
    local citation = {
    ...
    if item.mode == 'SuppressAuthor' then citation['suppress-author'] = true end
...
function Meta(meta)
  if string.match(FORMAT, 'docx') then config.format = 'docx' end
end

function Inlines_replace_cites(inlines)
  if not config.format then return inlines end
  for k, v in pairs(inlines) do
    if v.t == 'Cite' then inlines[k] = zotero_ref(v) end
  end
  return inlines
end
